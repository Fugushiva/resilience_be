...
export function KitResultPage() {
    ...
    if (result) {
      const pct = Math.round(
        (result.essentialCount /
          Math.max(result.categories.flatMap((c) => c.items).length, 1)) *
          100
      );
  ...
  const readinessScore = Math.round(
    (essentialCount / Math.max(totalItems, 1)) * 100
  );
  const totalPersons =
    result.profile.adults + result.profile.children + result.profile.teens;

  // ── URL de partage ────────────────────────────────────────────────────────
  // On utilise l'ID persistant si dispo, sinon le fallback query-params
  const shareUrl =
    getShareUrl() ||
    `${SITE_URL}/kit?score=${readinessScore}&persons=${totalPersons}&hours=${coverageHours}&essentials=${essentialCount}`;
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          ...
          text: `J'ai configuré mon kit d'urgence 72h sur Survikit. ${readinessScore}% de couverture pour ${totalPersons} personne${
            totalPersons > 1 ? "s" : ""
          }.`,
          url: shareUrl,
        });
  ...
  return (
    <>
       ...
       * Injecté côté client via dangerouslySetInnerHTML.
       */}
          ...
          {sync.shareId && (
            ...
            >
              <div className="min-w-0">
                <p className="text-xs text-ink-muted mb-1">Lien de sauvegarde</p>
